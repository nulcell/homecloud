...
- **Transparent WireGuard encryption** (`encryption.enabled: true`, `type: wireguard`) — every cross-node *pod-to-pod* packet is encrypted via a Cilium-managed `cilium_wg0` interface. No-op while you're single-node; live the moment a second node joins. ~10–15% throughput overhead at line rate; negligible on a homelab. **`nodeEncryption: false`** is intentional: when on, host-originated packets to remote pods are routed through `cilium_wg0`, which fights with `bpf.hostLegacyRouting: true` (next bullet) and silently breaks kube-apiserver → extension APIService discovery (`v1beta1.metrics.k8s.io`, Rancher's `v1.ext.cattle.io`, etc.). The trade-off is that host-network pods crossing nodes (Cilium agent, node-exporter, Hubble) traverse the LAN in plaintext — acceptable on a trusted home LAN.
- **`bpf.hostLegacyRouting: true`** — required for the kube-apiserver (host netns) to reach pod IPs on other nodes. Cilium's default eBPF host routing bypasses the kernel routing table that holds the direct-pod-CIDR routes, so without this any aggregated APIService whose pod lives on a different node times out. Documented as a [Talos/Cilium compatibility caveat](https://docs.siderolabs.com/talos/v1.13/networking/kubespan/#cilium-compatibility-limitations). Paired with `--enable-aggregator-routing=true` on the apiserver (see [talos/patches/controlplane.yaml](../talos/patches/controlplane.yaml)).
