...
- **Transparent WireGuard encryption** (`encryption.enabled: true`, `type: wireguard`, `nodeEncryption: true`) — every cross-node pod-to-pod and host-network packet is encrypted via a Cilium-managed `cilium_wg0` interface. No-op while you're single-node; live the moment a second node joins. ~10–15% throughput overhead at line rate; negligible on a homelab.
