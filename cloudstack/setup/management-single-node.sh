...
CLOUDBR1_IP=$(ip a show cloudbr1 | grep -i "inet " | awk '{print $2}' | cut -d'/' -f1)
...
# Configure default CPU mode to host-model
grep -qxF 'guest.cpu.mode=host-model' /etc/cloudstack/agent/agent.properties || echo 'guest.cpu.mode=host-model' | tee -a /etc/cloudstack/agent/agent.properties
