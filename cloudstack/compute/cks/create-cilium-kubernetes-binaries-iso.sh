...
if [ $# -lt 6 ]; then
    echo "Invalid input. Valid usage: ./create-kubernetes-binaries-iso.sh OUTPUT_PATH KUBERNETES_VERSION CNI_VERSION CRICTL_VERSION CILIUM_VERSION DASHBOARD_YAML_CONFIG BUILD_NAME [ARCH] [ETCD_VERSION]"
    echo "eg: ./create-kubernetes-binaries-iso.sh ./ 1.34.2 1.8.0 1.34.0 1.18.2 7.14.0 cks-v1.34.2-cilium amd64 3.5.0"
    exit 1
fi
...
CILIUM_VERSION="${5}"
echo "Downloading cilium ${CILIUM_VERSION}..."
...
echo "Downloading dashboard ${DASHBOARD_YAML_CONFIG}"
dashboard_conf_file="${working_dir}/dashboard.yaml"
# helm repo add kubernetes-dashboard https://kubernetes.github.io/dashboard/
# helm template kubernetes-dashboard kubernetes-dashboard/kubernetes-dashboard --version ${DASHBOARD_VERSION} --create-namespace --namespace kubernetes-dashboard > ${dashboard_conf_file}
curl -sSL ${DASHBOARD_YAML_CONFIG} -o ${dashboard_conf_file}
